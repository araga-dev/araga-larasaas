...
class UsersTable
{
    ...
    {
        return $table
            ->modifyQueryUsing(fn(Builder $query) => $query->with(['ownedOrganizations', 'adminOrganizations', 'memberOrganizations']))
            ->columns([
                ...
                TextColumn::make('organizations')
                    ->label('Contas')
                    ->formatStateUsing(fn($state, $record) => $record->organizations->map(fn($acc) => $acc->name)->join(', ')),
                TextColumn::make('organization_roles')
                    ->label('Papéis nas contas')
                    ->formatStateUsing(fn($state, $record) => $record->organizations->map(function ($acc) {
                        $roles = [];
                ...
                TextColumn::make('organization_status')
                    ->label('Status nas contas')
                    ->formatStateUsing(fn($state, $record) => $record->organizations->map(fn($acc) => $acc->pivot->status)->unique()->join(', ')),
            ...
            ->filters([
                ...
                SelectFilter::make('organizations')
                    ...
                    ->multiple()
                    ->preload(),
                SelectFilter::make('organization_role')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return match ($data['value'] ?? null) {
                            'owner' => $query->whereHas('organizations', fn(Builder $q) => $q->wherePivot('is_owner', true)),
                            'admin' => $query->whereHas('organizations', fn(Builder $q) => $q->wherePivot('is_admin', true)),
                            'user' => $query->whereHas('organizations', fn(Builder $q) => $q->wherePivot('is_owner', false)->wherePivot('is_admin', false)),
                            default => $query,
                        };
                    }),
                /*  SelectFilter::make('organization_status')
                    ->label('Status nas contas')
                    ->options([
                        'active' => 'Ativo',
                        'suspended' => 'Suspenso',
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        ...
                        return $query->whereHas('organizations', function (Builder $q) use ($status) {
                            $q->where('organization_user.status', $status);
                        });
                    }),*/
