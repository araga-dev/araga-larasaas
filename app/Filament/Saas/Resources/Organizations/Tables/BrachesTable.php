...
class BrachesTable
{
    public static function configure(Table $table, $organization): Table
    {
        return $table
            ->heading('Filiais')
            ->modifyQueryUsing(fn(Builder $query) => $query->where('organization_id', $organization->id))
            ->columns([
            ...
            ->actions([
                EditAction::make()
                    ->mutateFormDataUsing(function (array $data) {
                        // normaliza is_primary: evita colisão no UNIQUE (organization_id, is_primary)
            ...
            ->headerActions([
                CreateAction::make()
                    ->mutateFormDataUsing(function (array $data) use ($organization) {
